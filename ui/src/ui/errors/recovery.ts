...
const RATE_LIMIT_RE = /rate[_ ]limit|too many requests|429|quota exceeded|resource[_ ]exhausted/i;
const OVERLOADED_RE = /overloaded|overload|service unavailable|503/i;
const TIMEOUT_RE = /timeout|timed out|deadline exceeded|context deadline exceeded/i;
const BILLING_RE = /payment required|insufficient credits|billing|plan|402/i;
const AUTH_RE =
  /invalid[_ ]?api[_ ]?key|unauthorized|forbidden|auth|token|expired|credentials/i;
const CONTEXT_RE =
  /context (?:length|window)|context overflow|prompt is too long|request too large|413/i;
const COMPACTION_RE = /compaction failed|auto-compaction|summarization failed/i;
const FORMAT_RE = /invalid request format|string should match pattern|tool_use_id|tool_use\.id/i;
const IMAGE_DIMENSION_RE = /image dimensions exceed|max allowed size/i;
const IMAGE_SIZE_RE = /image exceeds .*mb/i;

export function classifyRecoveryCategory(raw?: string): RecoveryCategory {
  const message = (raw ?? "").trim();
  if (!message) return "unknown";
  if (IMAGE_DIMENSION_RE.test(message)) return "image_dimensions";
  if (IMAGE_SIZE_RE.test(message)) return "image_too_large";
  if (COMPACTION_RE.test(message)) return "compaction_failed";
  ...
  if (FORMAT_RE.test(message)) return "format";
