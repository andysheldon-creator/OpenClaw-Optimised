...
describe("statusCommand", () => {
  it("prints JSON when requested", async () => {
    ...
    expect(payload.sessions.paths[0]).toBe("/tmp/sessions.json");
    expect(payload.sessions.defaults.model).toBeTruthy();
    expect(payload.sessions.defaults.contextTokens).toBeGreaterThan(0);
    expect(payload.sessions.recent[0].percentUsed).toBe(50);
    expect(payload.sessions.recent[0].remainingTokens).toBe(5000);
    expect(payload.sessions.recent[0].flags).toContain("verbose:on");
    expect(payload.heartbeat.defaultAgentId).toBe("main");
    expect(payload.heartbeat.agents.length).toBeGreaterThan(0);
  ...
  it("aggregates sessions across agents", async () => {
