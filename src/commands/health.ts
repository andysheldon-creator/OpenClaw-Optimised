...
const resolveChannelAccountBindings = (cfg: ReturnType<typeof loadConfig>) => {
  const map = new Map<string, Map<string, string[]>>();
  const bindings = listBindings(cfg);
  debugHealth("bindings.count", bindings.length);
  for (const binding of bindings) {
    ...
    const match =
      binding.match && typeof binding.match === "object"
        ? (binding.match as { channel?: unknown; accountId?: unknown })
        : undefined;
    const channelId = typeof match?.channel === "string" ? match.channel.trim() : "";
    const accountId = typeof match?.accountId === "string" ? match.accountId.trim() : "";
    if (!channelId || !accountId || accountId === "*") continue;
    debugHealth("binding", { agentId, channelId, accountId });
...
const resolvePreferredAccountId = (params: {
  accountIds: string[];
...
function listBindings(cfg: ReturnType<typeof loadConfig>) {
...
export const formatHealthChannelLines = (
  ...
  for (const channelId of channelOrder) {
    ...
    const configured =
      typeof baseSummary.configured === "boolean" ? baseSummary.configured : null;
    if (configured === false) {
...
export async function getHealthSnapshot(params?: {
  ...
  const channelBindings = resolveChannelAccountBindings(cfg);
  ...
  const heartbeatSeconds = defaultAgent?.heartbeat.everyMs
    ...
    : 0;
  const sessions = defaultAgent?.sessions ?? buildSessionSummary(
  ...
  for (const plugin of listChannelPlugins()) {
    ...
    const boundAccountIdsAll = Array.from(
      new Set(
        Array.from(channelBindings.get(plugin.id)?.values() ?? []).flatMap((ids) => ids),
      ),
    );
    ...
    for (const accountId of accountIdsToProbe) {
      ...
      const bot = probeRecord && typeof probeRecord.bot === "object" ? (probeRecord.bot as { username?: string | null }) : null;
      if (bot?.username) {
