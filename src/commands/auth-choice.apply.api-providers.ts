...
export async function applyAuthChoiceApiProviders(
  ...
  if (authChoice === "huggingface-api-key") {
    ...
    if (
      !hasCredential &&
      params.opts?.token &&
      params.opts?.tokenProvider === "huggingface"
    ) {
    ...
    // When :cheapest or :fastest is selected, provider is locked (router picks). Any optional
    // "prefer specific backend" step must be shown only when !isHuggingfacePolicyLocked(selectedModelRef).
