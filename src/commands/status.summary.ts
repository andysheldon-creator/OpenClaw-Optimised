...
export async function getStatusSummary(): Promise<StatusSummary> {
  ...
  const allSessions: SessionStatus[] = [];
  const byAgent = agentList.agents.map((agent) => {
    const storePath = resolveStorePath(cfg.session?.store, { agentId: agent.id });
    paths.add(storePath);
    const store = loadSessionStore(storePath);
    const sessions = buildSessionRows(agent.id, storePath, store);
  ...
  const sessions = allSessions.sort((a, b) => (b.updatedAt ?? 0) - (a.updatedAt ?? 0));
  const recent = sessions.slice(0, 10);
  const totalSessions = byAgent.reduce((sum, agent) => sum + agent.count, 0);
