...
function shouldFallbackToAuthHeuristics(err: unknown): boolean {
	if (!(err instanceof Error)) {
		return false;
	}

	const code = (err as { code?: unknown }).code;
	if (code === "MODEL_AVAILABILITY_UNAVAILABLE") {
		return true;
	}

	const lower = `${err.name}: ${err.message}`.toLowerCase();
	return (
		lower.includes("availability unavailable") ||
		lower.includes("availability unsupported") ||
		lower.includes("availability unreliable")
	);
}
...
export async function loadModelRegistry(cfg: OpenClawConfig) {
	...
	try {
		...
		// Some providers can report availability as unsupported/unreliable.
