...
function replaceMarkers(content: string): string {
  ...
  if (!/external_untrusted_content/ui.test(folded)) {
    return content;
  }
  const replacements: Array<{ start: number; end: number; value: string }> = [];
  const patterns: Array<{ regex: RegExp; value: string }> = [
    // Add /u flag to all regex patterns for proper Unicode matching
    { regex: /<<<EXTERNAL_UNTRUSTED_CONTENT>>>/gui, value: "[[MARKER_SANITIZED]]" },
    { regex: /<<<END_EXTERNAL_UNTRUSTED_CONTENT>>>/gui, value: "[[END_MARKER_SANITIZED]]" },
  ];
