---
name: session-memory
description: "Save session context to memory with LLM summarization when /new command is issued"
...
Automatically saves session context to your workspace memory when you issue the `/new` command. Uses LLM to generate a structured summary for better future recall.
