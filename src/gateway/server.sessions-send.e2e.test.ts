...
beforeAll(async () => {
  ...
  prevGatewayToken = process.env.EASYHUB_GATEWAY_TOKEN;
  gatewayPort = await getFreePort();
  process.env.EASYHUB_GATEWAY_PORT = String(gatewayPort);
  process.env.EASYHUB_GATEWAY_TOKEN = "test-token";
...
afterAll(async () => {
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.EASYHUB_GATEWAY_TOKEN = prevGatewayToken;
  }
});
...
describe("sessions_send gateway loopback", () => {
  it("returns reply when lifecycle ends before agent.wait", async () => {
    ...
    const tool = createEasyHubTools().find((candidate) => candidate.name === "sessions_send");
    if (!tool) {
