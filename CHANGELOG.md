...
- Security: redact channel credentials (tokens, passwords, API keys, secrets) from `config.get` gateway responses so the Control UI and WebSocket clients never receive plaintext secrets.
