...
const { wrapExternalContent, detectSuspiciousPatterns } = require('./dist/security/external-content.js');
...
testCases.forEach((test, idx) => {
  ...
  const containsOriginalMarkers = wrapped.includes('<<<EXTERNAL_UNTRUSTED_CONTENT>>>') ||
                                 wrapped.includes('<<<END_EXTERNAL_UNTRUSTED_CONTENT>>>');
  const containsSanitized = wrapped.includes('[[MARKER_SANITIZED]]') ||
                           wrapped.includes('[[END_MARKER_SANITIZED]]');
