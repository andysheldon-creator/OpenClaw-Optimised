...
If you don’t set a provider explicitly, Clawdbot resolves the memory search
provider in this order: per‑agent config → defaults → **OpenAI if `OPENAI_API_KEY`
is set, otherwise Gemini if `GEMINI_API_KEY`/`GOOGLE_API_KEY` is set**. If neither
key is available, memory search stays disabled until you configure it.
